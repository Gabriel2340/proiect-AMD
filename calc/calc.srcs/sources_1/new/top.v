...
module top(
    ...
    MainControl main_control_1(.op(instr[5:0]), .din(instr[31:26]), .ZERO(ZERO), // TODO: main control
                .REGDST(REGDST), .REGWRITE(REGWRITE), .EXTOP(EXTOP), .ALUSRC(ALUSRC),
                .ALUOP(ALUOP), .MEMWRITE(MEMWRITE), .MEM2REG(MEM2REG));
    ExtSign ext_sign_1(.din(instr[15:0]), .dout(ext_dout)); // TODO: add EXTOP wire
